...
static class Util
{
	...
	/// <param name="paramName">The name of the parameter to mention in the exception. If left unspecified, defaults to the caller argument expression of <paramref name="id"/>.</param>
	...
	internal static void ThrowIfInvalidId(string? id, [CallerArgumentExpression("id")] string? paramName = null, bool allowNull = false)
	{
		if (id is null)
		{
			if (allowNull) return;
		...
		if (id.Length == 0)
		{
			throw new ArgumentException($"{paramName ?? "An ID"} is an empty string.", paramName);
		}
		foreach (char @char in id)
		{
			if (@char is < '0' or > '9') throw new ArgumentException(paramName is null ? "Invalid ID." : $"{paramName} is not a valid ID.", paramName);
		}
	}
}
